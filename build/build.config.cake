...
class BuildScheme
{
    ...
    public string CommonConfigFolder { get; set; }
    public string CommonConfigFile { get; set; }
    ...
    {
        return $"{GetRootOutputFolder()}/{GetProjectOutputRootFolder(project.ProjectType)}/{project.ProjectName}";
    }
...
void LoadBuildScheme(string outputRootPath, string projectsFile, string buildConfigurationFile)
{
    ...
    buildScheme.WebSites.ForEach(x => { if (projects.WebSites.TryGetValue(x.ProjectName, out var metadata)) x.ProjectMetadata = metadata; else throw new KeyNotFoundException($"WebSite {x.ProjectName} not found in projects file");});
    buildScheme.WebApplications.ForEach(x => { if (projects.WebApplications.TryGetValue(x.ProjectName, out var metadata)) x.ProjectMetadata = metadata; else throw new KeyNotFoundException($"WebApplication {x.ProjectName} not found in projects file");});
    buildScheme.WindowsApplications.ForEach(x => { if (projects.WindowsApplications.TryGetValue(x.ProjectName, out var metadata)) x.ProjectMetadata = metadata; else throw new KeyNotFoundException($"WindowsApplication {x.ProjectName} not found in projects file");});
    buildScheme.Databases.ForEach(x => { if (projects.Databases.TryGetValue(x.ProjectName, out var metadata)) x.ProjectMetadata = metadata; else throw new KeyNotFoundException($"Database project {x.ProjectName} not found in projects file");});
